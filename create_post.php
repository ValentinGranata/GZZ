<?php 
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (empty($title) || empty($description) || empty($category) || empty($email)) {
            $message = 'Inserisci tutti i dati per procedere!';
        } else {
            $check_query = "SELECT * FROM Startup WHERE email = '" . $email . "';";
            $check_result = $con->query($check_query);

            if ($check_result->num_rows > 0) {
                $message = "Esiste già una startup con questa email!";
            } else {
                ...
                if ($con->error) {
                    $message = "Errore sconosciuto durante la creazione della startup!";
                } else {
                    ...
                    $message = "Startup creata con successo!";
                }
            }
        }
    }
